...
MULTIMODAL_MODEL = "gpt-4o-mini" # Changed for cost optimization (Vision/Multimodal)
SUMMARY_MODEL = "gpt-4o-mini" # Changed for cost optimization (Text Summary)
...
def get_llm_description(image_b64: str, prompt: str) -> str:
    """Sends image to GPT-4o-mini for detailed description."""
...
def audit_single_policy_with_llm(image_b64: str, policy_data: Dict) -> Dict:
    """
    Sends a single policy AND the image to LLM for compliance checking.
    It returns a minimal dictionary with 'Policy', 'Breached', and 'Violation' fields.
    """
    ...
    # Note: We still send the full policy_data so the model knows the context, but ask for minimal output.
    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            temperature=0.0,
            max_tokens=256, # Reduced max_tokens since the expected output is much smaller
            timeout=API_TIMEOUT
        )
        ...
        return {
            "Policy": policy_data['Policy'], # Only return policy name and error status
            "Breached": "error",
...
def audit_narrative(narrative_text: str, static_ratio: float, first_frame_b64: str, policy_list: List[Dict]) -> List[Dict]:
    ...
    # 3. Incorporate LLM Results (MERGING MINIMAL OUTPUT)
    for result in llm_audit_results:
        ...
        if isinstance(result, dict) and policy_name and policy_name in final_report_map:
            # Update the map with the LLM's Breached and Violation status (Category/Description come from step 2)
